...
@output = IO.popen("iostat -Nxk 1")
    while line = @output.gets do
        if  ( line =~ /^(Linux|Time:|avg-cpu|Device| |$)/ )
            ...
            puts "PUTVAL " + HOSTNAME.chomp.split("_").first + "/iostat/" + device + "/rrqm " + "N:" + rrqm_sec
            puts "PUTVAL " + HOSTNAME.chomp.split("_").first + "/iostat/-" + device + "/wrqm " + "N:" + wrqm_sec
            puts "PUTVAL " + HOSTNAME.chomp.split("_").first + "/iostat/" + device + "/rs " + "N:" + r_s
            puts "PUTVAL " + HOSTNAME.chomp.split("_").first + "/iostat/" + device + "/ws " + "N:" + w_s
            puts "PUTVAL " + HOSTNAME.chomp.split("_").first + "/iostat/" + device + "/rsec " + "N:" + rsec
            puts "PUTVAL " + HOSTNAME.chomp.split("_").first + "/iostat/" + device + "/wsec " + "N:" + wsec
            puts "PUTVAL " + HOSTNAME.chomp.split("_").first + "/iostat/" + device + "/avgrqsz " + "N:" + avgrq_sz
            puts "PUTVAL " + HOSTNAME.chomp.split("_").first + "/iostat/" + device + "/avgqusz " + "N:" + avgqu_sz
            puts "PUTVAL " + HOSTNAME.chomp.split("_").first + "/iostat/" + device + "/await " + "N:" + await
            puts "PUTVAL " + HOSTNAME.chomp.split("_").first + "/iostat/" + device + "/svctm " + "N:" + svctm
        end
    end
